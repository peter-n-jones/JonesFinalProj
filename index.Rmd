...
regressions <- SDM_freedomlevel_internet_successrate |>
  group_by(press_freedom) |>
  summarize(intercept = coef(lm(success_rate ~ bench_dist))[[1]],
            slope = coef(lm(success_rate ~ bench_dist))[[2]])
...
p_values <- lapply(models, function(model) extract_p_values(model))

# Add p-values to the regressions data frame
regressions$p_value <- unlist(p_values)
regressions
...
```{r}
#New data frame with differences between repressed and free

differences_repressed <- SDM_freedomlevel_internet_successrate |>
  group_by(bench_dist) |>
  summarize(diff_0_2 = success_rate[press_freedom == 0] - success_rate[press_freedom == 2])

# View the updated differences_repressed dataframe
print(differences_repressed)

ggplot(differences_repressed, aes(x = bench_dist, y = diff_0_2)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Difference in SDM Success Rates, Supressed vs. Free",
       x = "Years from Benchmark",
       y = "Mean Difference Suppressed - Free") +
  theme_minimal()  #
...
# That is, does press freedom have a significant differential effect relative to internet takeup?

# step one: take mean of suppressed and semi-surpressed rates
# step two: plot against free
#step three: see if difference significant
...
# Is difference stat sig?

coef(summary(mod_free))[,"Pr(>|t|)"]
coef(summary(mod_unfree))[,"Pr(>|t|)"]
